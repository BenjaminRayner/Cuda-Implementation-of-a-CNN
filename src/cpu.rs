...
fn convolution_layer(input: &InputMatrix, conv_filters: &ConvLayer, outputs: &mut ConvOutput) {
    ...
    for (filter, out) in conv_filters.0.iter().zip(outputs.0.iter_mut()) { // Iterates over conv_filters & outputs for each neuron        
        ...
        for i in (0..INPUT_DIM).step_by(FILTER_DIM) {                                  // Vertical index of pixel (step by 5 because of cell size)
            for j in (0..INPUT_DIM).step_by(FILTER_DIM) {                              // Horizontal index of pixel (step by 5 because of cell size)
                // Dot product of 5x5 cell and 5x5 convolution matrix
                let prod: f64 = (0..FILTER_DIM)
                    .flat_map(move |x| {                                                    // Vertical index of pixel in cell
                        (0..FILTER_DIM).map(move |y| input.0[i + x][j + y] * filter[x][y])  // Horizontal index of pixel in cell 0 to 5. i+x & j+y are the absolute pixel coordinates. (j being the horizontal index of the cell and y being the horizontal index of the pixel in the cell, etc.)
                    })
                    .sum();
                out[i / FILTER_DIM][j / FILTER_DIM] = prod;                                        // Store the result in 20x20 output matrix
            }
        }
    }
}
...
// Entry point for the CPU implementation
