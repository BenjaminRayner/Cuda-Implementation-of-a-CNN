...
// Reference our modules to allow use
#![warn(clippy::all)]
mod cnn;
mod cpu;
mod cuda;
...
fn for_each_input<F: FnMut(&InputMatrix) -> Result<OutputVec, Box<dyn Error>>>(
    ...
    for _ in 0..size {  // read the matrices
        ...
        let output = f(&InputMatrix(input))?;  // invoke the user's function with the input matrix
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut ctx = if use_cuda {
        Some(cuda::CudaContext::init(&cnn)?)                // <---- Need to implement init
    } else {
        None
    };
    ...
    for_each_input(  // for each input matrix in the file
        ...
        |input| {  // this closure gets run on each matrix
            ...
            let output = if use_cuda {
                ctx.as_mut().unwrap().compute(&input)?      // <---- Need to implement compute
            } else {
            ...
            if ctx.is_some() {
                ctx.as_mut().unwrap().reset()?;  // reset the buffers
            }
            Ok(output)
        },
    )?;
